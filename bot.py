...
def handle_start(message):
    ...
    if code in all_links:
        ...
        for file_id in all_links[code]:
            try:
                sent = bot.copy_message(chat_id=user_id, from_chat_id='-1002425319175', message_id=file_id)  #here your databse channel id
...
def handle_media(message):
    if message.from_user.id in admin_ids:
        ...
        if len(uploads) >= 25:
            ...
            return
        msg = bot.copy_message(chat_id='-1002425319175', from_chat_id=message.chat.id, message_id=message.message_id) #here your databse channel id 
